...
function expandedTriangle(vertices) {
...
function patchMatrix(patch, phase) {
  ...
  const u = vertex => SOURCE_Y + SOURCE_RADIUS * Math.cos(vertex.latitude) * textureCoordinate(vertex.longitude - phase);
  const u1 = u(first);
  const u2 = u(second);
  const u3 = u(third);
...
export function createSphere(svg, { imageHref, id, duration = 90, animated = true }) {
  ...
  function createPatch(vertices, name, details) {
    ...
    const patch = { vertices, image, polygon, group, ...details };
  ...
  const canRun = () => playing && visible && !reducedMotion.matches && !document.hidden && !disposed;
  let hiddenColumn = -1;
  const paint = () => {
  ...
  function tick(time) {
    ...
    previousTime = time;
    if (time - previousPaint >= 1000 / 24) { paint(); previousPaint = time; }
  ...
  return {
    setPhase(radians) { phase = ((radians % TAU) + TAU) % TAU; paint(); },
    setAnimated(value) { playing = value; sync(); },
    setDuration(value) { seconds = value; },
    destroy() {
      disposed = true;
